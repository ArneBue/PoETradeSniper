...
class PoEAPI:
	...
	def __init__(self, startID, poeninjaapi):
		self.changeID = startID
		self.poeninjaapi = poeninjaapi
		...
		for stash in stashes:
			...
			for itemIterator in items:
				if itemIterator.get('league') == config.get('Config', 'league'):
					item = itemhelper.normalizeItem(itemIterator, stashName, accountName, lastCharacterName, self.poeninjaapi)
					self.filter(item)                    
